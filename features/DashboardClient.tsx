...
import { 
  FaChartBar, FaExclamationCircle, FaRegClock, 
  FaUsers, FaUserSlash, FaPlus, FaTrash, 
  FaEdit
} from 'react-icons/fa';
...
export const DashboardClient = () => {
  ...
  const onTypeSubmit: SubmitHandler<IMembershipTypeInput> = (data) => {
    if(editingPlanId){
      setPlans((prev) => {
        return prev.map(plan => plan.id === editingPlanId ? {...plan, ...data} : plan)
      })
    ...
    <div className="p-6">
      ...
      <div className="flex flex-wrap items-center justify-between mb-5">
        ...
        <div className='flex space-x-3'>
          <Button size="sm" startIcon={<FaPlus />} onClick={() => { setModalMode('member'); setIsOpen(true); }}>
            Add Member
          </Button>
          <Button size="sm" startIcon={<FaPlus />} onClick={() => { setModalMode('type'); setIsOpen(true); }}>
            Membership
          </Button>
        </div>
      </div>
      ...
      <Tile 
        ...
        <Tile 
          ...
          icon={<FaChartBar className="text-gray-800 size-6 dark:text-white/90" />} 
        />
        ...
        <Tile 
          ...
          icon={<FaUserSlash className="text-gray-800 size-6 dark:text-white/90" />} 
        />
      </div>
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} className="max-w-4xl p-6">
        <h2 className="mb-6 text-xl font-bold text-gray-800 dark:text-white">
          {modalMode === 'member' ? 'Add New Member' : 'Manage Membership Plans'}
        </h2>
        ...
        {modalMode === 'member' ? (
          ...
          <form onSubmit={handleMemberSubmit(onMemberSubmit)} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input register={regMember("name", { required: "Name is required" })} errorMessage={memberErr.name?.message as string} placeholder='Full Name' />
              </div>
              <div>
                <Label>Select Membership Plan <span className="text-red-500">*</span></Label>
                <Select
                    id="membershipId"
                ...
                {memberErr.membershipId && (
                    <p className="text-red-500 text-xs mt-1">{memberErr.membershipId.message}</p>
                )}

              </div>
              <div>
                <Label>Mobile Number <span className="text-red-500">*</span></Label>
                <Input register={regMember("mobile", { required: "Mobile required" })} errorMessage={memberErr.mobile?.message as string} placeholder='Mobile Number' />
              </div>
              <div>
          ...
          <div className="space-y-6">
            <form onSubmit={handleTypeSubmit(onTypeSubmit)} className="p-4 border rounded-xl bg-gray-50 dark:bg-gray-900/50">
              <h3 className="text-sm text-gray-800 dark:text-white font-bold mb-4">CREATE NEW PLAN</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input placeholder="Months (e.g. 12)" register={regType("months", { required: "Required" })} errorMessage={typeErr.months?.message as string} />
                <Input placeholder="Price (e.g. 5000)" register={regType("price", { required: "Required" })} errorMessage={typeErr.price?.message as string} />
              </div>
            ...
            <div className="overflow-hidden border rounded-xl">
              <table className="w-full text-sm text-center">
                ...
                <tbody>
                  {plans.map(plan => (
                    <tr key={plan.id} className={`border-t text-gray-800 dark:text-white dark:border-gray-700 ${editingPlanId === plan.id ? "bg-yellow-100 dark:bg-yellow-900/30" : ""}`}>
                      <td className="p-4 capitalize">{plan.months} Months</td>
                      <td className="p-4">₹{plan.price}</td>
                      <td className="p-4 text-center space-x-10">
                        <button onClick={() => deletePlan(plan.id)} className="text-red-500 hover:scale-110 transition-transform">
                          <FaTrash />
                        </button>
                        <button onClick={() => handleEditPlan(plan)} className="text-green-500 hover:scale-110 transition-transform">
                          <FaEdit />
                        </button>
                      </td>
                    </tr>
